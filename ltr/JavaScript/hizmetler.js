...
document.querySelectorAll(".btn-uygula").forEach((btn) => {
  btn.addEventListener("click", function () {
    ...
    ).map((cb) => cb.getAttribute("data-kategori-hizmetler"));
...
function filtreleKategoriler(secilenKategoriler) {
  const satirlar = document.querySelectorAll("tbody tr"); // veya "#tableContent tr" eğer id eklediysen

  satirlar.forEach((satir) => {
    const kategori = satir.classList[0];
...
function toggleDropdown(button) {
  ...
  document.querySelectorAll(".hizmetler-filter-menu").forEach((otherMenu) => {
    if (otherMenu !== menu) {
...
document.addEventListener("click", function (e) {
  document
    .querySelectorAll(".hizmetler-filter-container")
    .forEach((container) => {
      if (!container.contains(e.target)) {
        const menu = container.querySelector(".hizmetler-filter-menu");
